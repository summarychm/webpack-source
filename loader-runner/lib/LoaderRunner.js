...
function runSyncOrAsync(fn, context, args, callback) {
	...
	context.async = function async() {
		...
		isSync = false;
		return innerCallback; // 返回loader同步返回方法
	};
	// 挂载loader同步返回方法
 ...
 * @param {Buffer} args
 * @param {Boolean} raw 是否raw
 */
function convertArgs(args, raw) {
	if (!raw && Buffer.isBuffer(args[0])) args[0] = utf8BufferToString(args[0]);
	else if (raw && typeof args[0] === "string") args[0] = new Buffer(args[0], "utf-8"); // eslint-disable-line
}
...
function iteratePitchingLoaders(options, loaderContext, callback) {
	// 在最后一个loader之后终止,abort after last loader
...
/** loader开始执行阶段,开始递归解析依赖
 * @param {*} options
 * @param {*} loaderContext
 * @param {*} callback
 */
function processResource(options, loaderContext, callback) {
	// 将loader索引设置为最后一个loader, set loader index to last loader
	loaderContext.loaderIndex = loaderContext.loaders.length - 1;

	var resourcePath = loaderContext.resourcePath;
	if (resourcePath) {
		loaderContext.addDependency(resourcePath); // 添加文件依赖
		// 读取文件
...
exports.runLoaders = function runLoaders(options, callback) {
	...
	// 准备loader对象,prepare loader objects
	...
	loaderContext.resourceQuery = resourceQuery; //当前文件的?后部分
	...
	// resource属性,get时自动追加resourceQuery,set时自动拆分resourcePath和resourceQuery
	...
	// request属性,获取被解析出来的request字符串。
	// 例"/abc/style-loader.js?xyz!/abc/css-loader.js!/abc/index.js?rrr"
